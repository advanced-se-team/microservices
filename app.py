...
@app.route('/cal/<object_id>')
def get_calendar_by_object_id(object_id):
    print(object_id)
    UCAS_CourseCalendar = leancloud.Object.extend('UCAS_CourseCalendar')
    cal = UCAS_CourseCalendar.create_without_data(object_id)
    cal.fetch()
    response = requests.get(cal.get('course_ics').url)
