...
class Calendar:
    ...
    def to_csv(self, filename='collsion.csv'):
        csv_result = "lesson_name,week,day,lesson_n,stu_name\n"
        for ev in self.events_number_formatted:
            csv_result+= f"{ev[1]}," + f"{ev[5]}," + f"{ev[6]}," + f"{ev[2]}," + f"于卓浩\n"
        with open(filename, 'w',encoding='utf8') as f:
            f.write(csv_result)
